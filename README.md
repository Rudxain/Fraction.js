# About this branch
This is my personal branch and it's only for the development of the `radix`/`base` feature (WIP). This is because [Infusion decided it's not worth implementing](https://github.com/infusion/Fraction.js/issues/6#issuecomment-120950346). However, I still hope this may be added in the future, so this branch also works as a Just-In-Case for *when* that happens (**if** it actually happens)
...
String toString([digitPlaces=15], [base=10])
---
Generates an exact string representation of the actual object. For repeated decimal places all digits are collected within brackets, like `1/3 = "0.(3)"`. For all other numbers, up to `digitPlaces` significant digits are collected - which includes trailing zeros if the number is getting truncated. However, `1/2 = "0.5"` without trailing zeros of course. If `base` (AKA `radix`) is any value other than `10`, then the returned string will be formatted in that positional numeral system. If `base = 2`, the digit places are called *"binary places"*, if `3` then *"ternary places"*, and so on. The `base` param was added in response to [this issue](https://github.com/infusion/Fraction.js/issues/6)
